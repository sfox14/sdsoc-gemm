		...
		float *ascale, float *bscale)
{
	...
	for (int i=0; i<(batch/bt); i++){
		...
		float sqerr = 0;
		for (int i=0; i<(bt*m*k); i++){
			float imm = Cf[i]; //1.0/(as*bs);
			float err = C[i]-imm;
			sqerr += (err*err);
			if (i%1000000 == 0) printf("%.6f		%.6f		error=%.6f\n", C[i], imm, err);
		}
...
int main(int argc, char** argv)
{
	...
	bscale = (float *) sds_alloc(sizeof(float)*btile);

	/* hardware inputs and outputs */
	Af = (data_t *) sds_alloc(int(sizeof(data_t)*max_arow*max_acol));
	Bf = (data_t *) sds_alloc(int(sizeof(data_t)*btile*max_acol*max_image)); // set to max_col because transpose
	Cf = (float *) sds_alloc(int(sizeof(float)*max_acol*btile*max_image)); // set to max_col because transpose
